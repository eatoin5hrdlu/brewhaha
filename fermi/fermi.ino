/*
 * Fermentation Vessel controller
 */
/*
 * WIRING:
 *  +5V                  Orange
...
void setup()
{
	...
	{
		id = 'l';	// Default Fermentaphone ID 'f'
...
//        lux_init(2591);
	...
	once = true;
	auto_temp = false;   // Maintain Temperature Control
	auto_valve = false;  // Maintain Flow
	auto_mixer = false;  // Cycle magnetic mixer to avoid stalled stir-bar
...
int currentbubble;
	...
	if (streaming && (cnt_stream++ % 10 == 0))
	{
	    currentbubble = analogRead(BUBBLE);
	    if (abs(currentbubble - lastbubble) > 100 ) {
	       	    printTermInt("b",currentbubble);
	       	    lastbubble = currentbubble;
	    }
//	    printTermInt("t",get_temperature());
//	    printTermInt("id",id);
//	    printTermUInt("f",44); // luxRaw());
	}
