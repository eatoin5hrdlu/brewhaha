...
By default it runs the output example. use aplaymidi -l to get midi device numbers
python full.py --output 2
python full.py --input  3

MIDI TO PYGAME EVENTS:  pyEvents = pygame.midi.midis2events(midi_events, i.device_id)
"""
...
def input_main(device_id = None):
    ...
    while going:
        if i.poll():
            ...
            for (dat, bit) in midi_events:
                if (dat[0] == 144 and dat[2] == 0) :
                    dat[0] = 128
                fp.write('%s,\n' % dat)
                if (not dat[0] in msgs) :
                    print(dat)
                    msgs.append(dat[0])
                if ( dat[1] == 60 ) :
                    print( dat )
